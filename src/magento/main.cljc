(ns magento.main
  "Kotodama WASM entrypoint for the Magento clean-room actor (L5) — Clojure port.
  ...
  facts (`magento.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "magento")
(def tier "L5")
...
(def entity-specs
  [{:entity "Order"      :plural "orders"      :id-prefix "magento_ord"
    :fields [:entityId :incrementId :state :status :customerId :customerEmail :grandTotal :subtotal]
    :required [:state :status]
    :coerce {:entityId :int :customerId :int :grandTotal :float :subtotal :float}
    :refs {:customerId "Customer"}}
   {:entity "Product"    :plural "products"    :id-prefix ""
    :fields [:id :sku :name :price :weight :status :visibility :typeId :attributeSetId]
    :required [:id :sku]
    :coerce {:id :int :price :float :weight :float :status :int :visibility :int :attributeSetId :int}
    :refs {}}
   {:entity "Customer"   :plural "customers"   :id-prefix ""
    :fields [:id :email :firstname :lastname :groupId :storeId :websiteId]
    :required [:id :email]
    :coerce {:id :int :groupId :int :storeId :int :websiteId :int}
    :refs {}}
   {:entity "Category"   :plural "categories"  :id-prefix ""
    :fields [:id :parentId :name :isActive :position :level :path :productCount]
    :required [:id :name]
    :coerce {:id :int :parentId :int :isActive :bool :position :int :level :int :productCount :int}
    :refs {}}
   {:entity "Invoice"    :plural "invoices"    :id-prefix "magento_inv"
    :fields [:entityId :incrementId :orderId :state :grandTotal :subtotal]
    :required [:state :grandTotal]
    :coerce {:entityId :int :orderId :int :state :int :grandTotal :float :subtotal :float}
    :refs {:orderId "Order"}}
   {:entity "Shipment"   :plural "shipments"   :id-prefix "magento_shi"
    :fields [:entityId :incrementId :orderId :storeId :totalQty :totalWeight]
    :required [:totalQty :totalWeight]
    :coerce {:entityId :int :orderId :int :storeId :int :totalQty :float :totalWeight :float}
    :refs {:orderId "Order"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"magento.<Entity>/<field>\" v ...}. The datomic
...
(defn handle-create [store entity data]
  (let [{:keys [fields required coerce id-prefix]} (spec-for entity)]
    (or (some-> (reject-unknown data fields) (vector 400))
        ...
        (let [base (if (seq id-prefix) {:id (new-id id-prefix)} {})
...
(defn healthz [] [{:status "ok" :actor "magento-compat" :tier tier :entities entities} 200])
...
(defn start! [] :magento-compat/ready)
